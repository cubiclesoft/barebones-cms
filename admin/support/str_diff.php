...
class StrDiff
{
	 ...
	 * $compareCharacters - true to compare characters, and false to compare lines; this optional parameter defaults to false
	 */
	...
	private static function computeTable($sequence1, $sequence2, $start, $end1, $end2)
	{
		...
		for ($index1 = 1; $index1 <= $length1; $index1 ++)
		{
			...
			for ($index2 = 1; $index2 <= $length2; $index2 ++)
			{
				...
				if ($sequence1[$index1 + $start - 1] == $sequence2[$index2 + $start - 1])  $table[$index1][$index2] = $table[$index1 - 1][$index2 - 1] + 1;
				else  $table[$index1][$index2] = max($table[$index1 - 1][$index2], $table[$index1][$index2 - 1]);
			}
		}
	 ...
	 *
	 * $diff - the diff array
	 * $separator - the separator between lines; this optional parameter defaults to "\n"
	 */
	 ...
	 *
	 * $diff - the diff array
	 * $separator - the separator between lines; this optional parameter defaults to "<br>"
	 */
	 ...
	 *
	 * $diff - the diff array
	 * $indentation - indentation to add to every line of the generated HTML; this optional parameter defaults to ""
	 * $separator - the separator between lines; this optional parameter defaults to "<br>"
	 */
	public static function toTable($diff, $indentation = "", $separator = "<br>")
	{
		...
		while ($index < count($diff))
		{
			...
			$html .= $indentation . "\t<tr>\n";
			$html .= $indentation . "\t\t<td class=\"diff" . ($leftCell == $rightCell ? "Unmodified" : ($leftCell == "" ? "Blank" : "Deleted")) . "\">" . $leftCell . "</td>\n";
			$html .= $indentation . "\t\t<td class=\"diff" . ($leftCell == $rightCell ? "Unmodified" : ($rightCell == "" ? "Blank" : "Inserted")) . "\">" . $rightCell . "</td>\n";
			$html .= $indentation. "\t</tr>\n";
		}
	...
	/* Returns the content of the cell, for use in the toTable function. The parameters are:
	 *
	 * $diff - the diff array
